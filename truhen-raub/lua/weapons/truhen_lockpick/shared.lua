...
SWEP.PrintName = "Kisten Lockpick"
SWEP.Author = "DarkRP (Edit by Katakuri)"
SWEP.Instructions = ""
SWEP.Contact = ""
SWEP.Purpose = ""
...
SWEP.Spawnable = true
SWEP.AdminOnly = true
SWEP.Category = "Kingdom:RP Kisten"
...
function SWEP:PrimaryAttack()
    if GetGlobalBool("Truhen_Cooldown", true) then return end
    ...
    if canLockpick ~= true and (
            trace.HitPos:DistToSqr(self:GetOwner():GetShootPos()) > 1000 or
            (not GAMEMODE.Config.canforcedooropen and ent:getKeysNonOwnable()) or
            (not ent:GetClass() == "truhe_raubbar" and not string.find(string.lower(ent:GetClass()), "vehicle") and (not GAMEMODE.Config.lockpickfading or not ent.isFadingDoor))
