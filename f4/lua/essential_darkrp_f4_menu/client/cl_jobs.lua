...
--[[
	in zeile 380 c.a muss ich die Job level eingeben, die benötigt is um den job zu Spielen
]]--

local white = Color(255, 255, 255)
local black = Color(0, 0, 0)
local orange = Color(211, 84, 0)

local defaultBlur = Color(10, 10, 10, 160)
local defaultBlurOutline = Color(0, 0, 0, 255)
...
local function openJobs()
	...
	for _, job in ipairs(RPExtraTeams) do
		...
		if essentialDarkRPF4Menu.displayItem then
			...
			function jobButton:Paint()
				...
				if job.level == nil then return end
				if LocalPlayer():getDarkRPVar("level") >= job.level then
					draw.SimpleText("Benötigtes Level: "..job.level, "edf_roboto20", self:GetWide() * .88, self:GetTall() / 2, Color(25,255,25),TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER )
				else 
					draw.SimpleText("Benötigtes Level: "..job.level, "edf_roboto20", self:GetWide() * .88, self:GetTall() / 2, Color(255,25,25),TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER )
				end
			end
	...
	if #validItems < 1 then
		...
		unavailableLabel:SetText('Nicht verfügbar mit deinen Beruf. . .')
